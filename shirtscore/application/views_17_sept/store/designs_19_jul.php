          ...
          <ul class="sidebar1">
                <?php if(isset($cat)): ?>
                    ...
                    <?php foreach ($design_category as $category): ?>                
                      <li><a <?php if($cat == $category->slug){echo 'class="active-category"';}?> href="<?php echo base_url() ?>store/designs/<?php echo $sort.'/'.$category->slug.'/'.$keyword; ?>" style="width:76%"><?php echo $category->category_name; ?></a></li>
                    <?php endforeach ?>
                <?php endif ?>
             </ul> 
          </div><!-- span12 -->
        ...
        <!-- crosoul slider -->
         <?php  if(!empty($best_sell)){ ?>
          <div id="slider1">
              ...
              <div class="viewport">
                <h3 align="center" style="color:#3b5998">BEST SELLER</h3>
                <ul class="overview">
               <?php
                foreach($best_sell as $sell){ ?>
                  <li><a href="<?php echo $sell->best_sell_link; ?>" title="<?php echo $sell->best_sell_title; ?>"><img src="<?php echo base_url(str_replace('./','',$sell->thumb_image));?>" style="margin-left:60px" /></a></li>
                  <?php } ?>
...
<p>You can upload your own custom designs and <a href="<?php echo base_url() ?>store/signup">begin selling</a> you custom designs on our site. You will receive a commission for every sale that includes your custom design or graphic. You can also sell your custom T-shirts through our storefront or create your own personal online store that only offers your cool creations, graphic designs, names, team or other logos. Sign up for a a free <a href="<?php echo base_url() ?>store/signup">T-shirt shop</a> with ShirtScore!</p>
