...
class Store extends CI_Controller {
		...
		$product_arr = array();
  		...
  		if ($_POST) {
			...
			$data['designs']=$this->store_model->get_design_ajax($design_category->id);
	  		...
	  		$data['products']=$this->store_model->get_result('products',array('product_status'=>1,'is_customized'=>0 ,'admin_custom'=>0), '',array('order','asc'));
	  		$i = 0;
	  		if ($data['products']) {
 ...
 //  		$this->load->view('templates/store_template', $data);			
	...
	public function facebook_ajax()
	{
		...
		$this->session->set_userdata('fb_login_user_info',$fb_data);
...
*/
			...
			$this->session->unset_userdata('fb_login_user_info');
	...
	public function open_store(){
		...
		if ($this->form_validation->run() == TRUE){
			...
			$user = customer_info();
			$this->send_store_open_email($user['firstname'], $user['lastname'], $user['email'],$this->input->post('store_name'),$this->input->post('store_link'));
			$this->session->set_flashdata('success_msg', '<strong>What Now?</strong><br> Your store is being reviewed and you will receive an email notification once your store is ready for sales.<br> In the meantime feel free to upload as many designs as you wish.<br> More designs equal more sales and more sales mean more revenue for you!');
	...
	public function template_for_resgistration($fname, $lname, $email, $password=""){
		...
		$data['template'] = '';
        $message = $this->load->view('email/email_registration', $data, TRUE);
		return $message;
	}
    ...
    public function template_for_store_open($fname, $lname, $email,$store_name, $store_link){
	  $data = array(
	                 'fname'=>$fname,
	                 'lname'=>$lname,
	                 'email'=>$email,
	  ...
	  $message = $this->load->view('email/store_open_registration',$data,TRUE);
	  return $message;
	}

	public function send_store_open_email($fname, $lname, $email, $store_name, $store_link){
		...
		$html = $this->template_for_store_open($fname, $lname, $email, $store_name, $store_link);
		$is_fail = $this->smtp_email->sendEmail($from, $to, $subject, $html);
	}
	...
	*/
	...
	public function payment(){
		...
		$expiry_date = sprintf("%02s", $_POST['card-expiry-month']) . substr($_POST['card-expiry-year'], 2);

	    $amounts = $this->session->userdata('grand_total');
		$amounts['shipping_handling'] = 4.95;
	    $amounts['total_amount'] = ($this->cart->total() - $amounts['discount']) + 4.95;
		
		$name_arr = explode(" ", trim($userdatasession['billing_name']), 2);

		$post_values = array(			
			"x_login"			=> "88rAy4P74",
			"x_tran_key"		=> "8x53M5Pc5L7hePwt",		
			"x_version"			=> "3.1",
			"x_delim_data"		=> "TRUE",
			"x_delim_char"		=> "|",
			...
			"x_type"			=> "AUTH_CAPTURE",
			"x_method"			=> "CC",
			"x_card_num"		=> trim($_POST['card-number']),
			"x_exp_date"		=> $expiry_date,
			"x_card_code"		=> trim($_POST['card-cvc']),		
			"x_amount"			=> $amounts['total_amount'],
			"x_description"		=> "Shirtscore Apparel",
			"x_currency_code"	=> "USD",
			"x_duplicate_window"=> "5",		
			"x_first_name"		=> $name_arr[0],
			"x_last_name"		=> $name_arr[1],
			"x_address"			=> trim($userdatasession['billing_address']),
			"x_state"			=> trim($userdatasession['state']),
			"x_zip"				=> trim($userdatasession['zip_code']),
			"x_email"			=> trim($userdatasession['email'])
		...
		$reference = $response_array[6];
		$card_code_response = $response_array[38];
		$avs_response = $response_array[5];
		
		if ($result == 1) {
			
			$response = $this->store_model->payment_update($amounts['total_amount'], $reference, $authorization_code);
			...
			redirect('store/cart_payment?e=' . urlencode($error));
	...
	public function payment_status($order_id = ''){
		...
		$data['latest_design'] = $this->store_model->get_limited_result('design', array('status'=>'1'),4);
	...
	public function order_info(){
		...
		if($order = $this->store_model->get_row('orders', array('order_id' => $order_id))){
			if(!empty($order->buyer_id)){
				$buyer_id = $this->store_model->get_row('buyers', array('id' => $order->buyer_id));
				  	if($buyer_id->email != $email){
				 		$this->session->set_flashdata('msg_info', 'Email Address doesn`t match in our service.');
			...
			$this->session->set_flashdata('msg_error', 'Invalid Order Id! Order doesn`t match in our service');
			redirect('store/order_info');
		}
...
// awais bhai
            ...
            if ($resp){             
                ...
                $this->sendmail_to_superadmin($token_new,$email,$name,$subject,$message);
                 ...
                 $html = "<em><strong>Hello</strong></em> <br>
                        <p>".$name." Thank you for contacting with us, <br> We have received your Query and we will respond back to you soon.</p>
                 ";
    ...
    public function fbshare($id=NULL, $url='', $type='design'){
	...
	public function fblike($id=NULL, $url=''){
        $data['result'] = $this->store_model->get_row('design', array('id' => $id));
        if ($url == '')
            $data['url'] = base_url().'store/design_info/'.$data['result']->slug;
        else
        ...
        $this->load->view('fb_like_test',$data);        
    }

	public function fbshare_product($id=NULL,$url=''){
    	$data['result'] = $this->store_model->get_row('products', array('id' => $id));
    
    	if ($url == '')
    		$data['url'] = base_url().'store/product_info/'.$data['result']->slug;
    	else
    		$data['url'] = $url;
		$this->load->view('fbshare_product',$data);		
	}
	// Waseem work

	 public function twtshare($id=NULL, $url='', $type='design'){
    	$data['result'] = $this->store_model->get_row('design', array('id' => $id));
    	if ($url == '')
    		$data['url'] = base_url().'store/design_info/'.$data['result']->slug;
    	else
    		$data['url'] = $url;
		$this->load->view('twtshare',$data);		
	}

	 public function gplus_share($id=NULL, $url='', $type='design'){
    	$data['result'] = $this->store_model->get_row('design', array('id' => $id));
    	if ($url == '')
    		$data['url'] = base_url().'store/design_info/'.$data['result']->slug;
    	else
    		$data['url'] = $url;
		$this->load->view('gplus_share',$data);		
	}
	 public function pinit_share($id=NULL, $url='', $type='design'){
    	$data['result'] = $this->store_model->get_row('design', array('id' => $id));
    	if ($url == '')
    		$data['url'] = base_url().'store/design_info/'.$data['result']->slug;
    	else
    		$data['url'] = $url;
		$this->load->view('pinit_share',$data);		
	}
	 public function tumbler_share($id=NULL, $url='', $type='design'){
    	$data['result'] = $this->store_model->get_row('design', array('id' => $id));
    	if ($url == '')
    		$data['url'] = base_url().'store/design_info/'.$data['result']->slug;
    	else
    		$data['url'] = $url;
		$this->load->view('tumbler_share',$data);		
	}
	...
	public function designs($sort_by='best-selling',$category_slug='-', $search='-',$offset=0){				
		...
		if($this->session->userdata('my_store')){
			...
			$data['best_sell'] = $this->store_model->get_result('best_seller',array('status'=>1),'', array('id','desc'));
			$data['design_category'] = $this->store_model->design_category();
	...
	//	print_r($product);
		...
		$data['latest_design'] = $this->store_model->get_limited_result('design', array('status'=>'1'),10); //new change
	...
	public function add_to_cart_custom($product_id = ''){
		...
		$check_back=$this->store_model->get_row('products',array('id'=>$product_id));
					if($check_back->not_back==1)
					{
						redirect('store/cart');
					}
		...
		if(count($new_arr_for_sizes) > 0){
	        ...
	        $i=0;
		   	...
		   	foreach($new_arr_for_sizes as $varr){
				...
				{
					$id_new_product=$this->store_model->insert_last_record($prod_idd);

					$cus_prod_info=$this->store_model->get_row('products',array('id'=>$id_new_product));
					$custom_product = array();
				}
				...
				if ($this->session->userdata('design_your_own_products')) {
					...
					{
						$custom_product_test = $this->session->userdata('design_your_own_products');
						$custom_product=array(
							'custom_product'	=>	$id_new_product,
							'product_used'		=>	$cus_prod_info->product_used,
							'design_id'			=>	$custom_product_test['design_id'],
							'texts'				=>	$custom_product_test['texts'],
							'product_image'		=>	$custom_product_test['product_image'],
							'back_image'		=>	$custom_product_test['back_image'],
							'product_title'		=>	$cus_prod_info->regular_name,
							'price'				=>	$cus_prod_info->price,
							'created'			=>	date('Y-m-d h:i:s')
						);
					}
            	...
            	$this->store_model->update('products',array('price'=>$price,'not_back'=>1),array('id'=>$custom_product['custom_product']));
            	$i++;
			}
		} 	
 ...
 //  	public function designs($sort_by='most-liked',$category_slug='-', $search='-',$offset=0){				
	...
	// }
  	...
  	public function product_info($slug=""){
  		...
  		$data['latest_design'] = $this->store_model->get_limited_result('design', array('status'=>'1'),10); //
				...
				if(count($new_arr_for_sizes) > 0){
					foreach($new_arr_for_sizes as $varr){
						...
						array(
				  		 ...
				  		 $size_details=$this->store_model->get_row('product_size_price',array('size_id'=>$size_id,'product_id'=>$products->id));
  ...
  public function wear_it($slug="",$product_id='')
	...
	public function ajax_col_imgs_multi($color_id = '',$design_slug='')
	{
		...
		{	$design=array('slug'=>$design_slug);
			...
			{	
				...
				$html='<div style="text-align:'.$position.';min-height: 95px;">
					<img style="max-height:130px; max-width:'.$width.'px;left:'.$left.'px;position: relative; top:'.$top.'px; z-index: 1;" src="'.base_url().'assets/uploads/designs/thumbnail/'.$data["design"]->design_image.'"/></div>';
				$html.='<img style="margin-top: -25px;" src="'.base_url().'assets/uploads/color_img/'.$color_img->main_image.'" /><br>';
				$designID=$data['design']->id;
				$designImage = $data['design']->design_image;
				$designDesc = $data['design']->description;
				$designTitle = $data['design']->design_title;;
				$share_pinit="'$designID','$designImage','$designDesc'";
				$share_tumbler = "'$designID','$designTitle','$designDesc'";
				$share_stumbleupon = "'$designID','$designTitle'";
				$html.='<div>SHARE THIS DESIGN:</div><div class="shareit">
                      <a onclick="fbshare('.$data["design"]->id.');" title="share on facebook" ids="facebook_share'.$data["design"]->id.'" href="javascript:void(0);" ><img src="../assets/front_theme/img/fb.png"></a>
                      <a onclick="twtshare('.$data["design"]->id.');" title="share on twitter" ids="twt_share'.$data["design"]->id.'" href="javascript:void(0);" ><img src="../assets/front_theme/img/tweet.png"></a>
                      <a onclick="gplus_share('.$data["design"]->id.');" title="share on google plus" ids="gplus_share'.$data["design"]->id.'" href="javascript:void(0);" ><img src="../assets/front_theme/img/gplus.png"></a>
                      <a onclick="pinit_share('.$share_pinit.');" title="share on Pinter" ids="pinit_share'.$data["design"]->id.'" href="javascript:void(0);" ><img src="../assets/front_theme/img/pin.png"></a>
                      <a onclick="tumbler_share('.$share_tumbler.');" title="share on tumbler" ids="tumbler_share'.$data["design"]->id.'" href="javascript:void(0);" ><img src="../assets/front_theme/img/tumblr.png"></a>
                       <a onclick="stumbleupon('.$share_stumbleupon.');" title="share on Stumbleupon" ids="stumbleupon'.$data["design"]->id.'" href="javascript:void(0);" ><img src="../assets/front_theme/img/stumbleupon.png"></a>
                    </div><br><br>';
				?>
             	<?php
				}
	...
	public function create_product_design_multi($design_slug = '', $product_id =''){
		$design=array('slug'=>$design_slug);
		$design_info=$this->store_model->get_row('design',$design);

		$design_id=$design_info->id;
		if(empty($product_id))
		{
			$data['single_product']=$this->store_model->wear_it_single_product($product_id);
		...
		$product_used = $this->store_model->get_row('products', array('id' => $product_id));
				...
				if(count($new_arr_for_sizes) > 0){
						...
						$design_price=$design_info->price;
						...
						array(
			  			...
			  			$data_design=array($design_id=>array('design_qty'=>1,'price'=>$design_info->price));		
	    				$cart_details=array(
						'Product_id' 	 => $custom_prod_id,
						'Product_Simple' 	 => FALSE,
				...
				if($custom_product['slug'] == '')
					...
					$data = array( 'id'      =>  $custom_prod_id,
				               'qty'     => $qty,
				               'price'   => number_format($price ,2),
				               'name'    => $product_used->regular_name,
				               'options' => $cart_details);
		...
		public function image_merged_multi($item = 0, $design_id = 0, $product_id = '',$color_id='')
	{
		...
		$bg_x_w = $bg_x_w - ($over_x_w/1.5);
		$bg_y_h = (int) ($data1[1]/3);
		$bg_y_h = $bg_y_h - ($over_y_h/1.5);
		$product_attr=$this->store_model->get_row('products',array('id'=>$product_id));
		if($product_attr->position=='left')
               $bg_x_w=50;
        if($product_attr->position=='right')
           $bg_x_w=250;
		imagealphablending($bg, true);
		imagesavealpha($bg, true);
		imagecopy($bg, $over, $bg_x_w, $bg_y_h, 0, 0, $over_x_w, $over_y_h);
 ...
 function login_with_facebook(){
		...
		$loginUrl='';
			...
			if ($user) {
			 ...
			 $loginUrl = $this->facebook->getLoginUrl(array(
			 	'scope' => 'email',
			 	'redirect_uri'=> base_url().'store/fb_login_check_new',
			 	'display'=>'popup'
			 	)
			 );
			 redirect($loginUrl);
	...
	public function fb_login_check_new(){			
		...
		if ($user){
			...
			if(!empty($user_profile)){
				...
				$this->session->set_userdata('fb_login_user_info',$profile);
			}


		}
		redirect('store/signup_form');
	}
