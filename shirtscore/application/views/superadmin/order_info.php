<style>
  .downloadpdf{color: rgb(42, 238, 144);
    font-size: 14px;
    text-align: right;
    float: right;
    margin-right: 60px;
      ...
      <div class="row-fluid ">     	
        ...
        <div class="span12">
          ...
          <div class="box paint color_0">
            <div class="title">
              <h4> <i class="icon-reorder"></i> <span>Order Info </span> <a href="<?php echo base_url('superadmin/order_pdf').'/'.$order_user_info->id?>" class="downloadpdf">Download order sheet</a>
    ...
    <td>
      ...
      if(!empty($custom_uplaod_img)){
        ...
        if(!empty($texts) && is_array($texts)){
          ...
          ?>
          <div class="row text-left"><a class="btn btn-info" style="color:#FFF;" href="<?php echo base_url('superadmin/merge_text_i/'.$row->order_id.'/'.$cart_detail->options->Product_id)?>" title="">Download Text</a></div>
          <?php
          echo '<div style="text-align:left; color:#FFF;"><a style="color:#FFF;" class="fancybox" href="#showtext'.$ii.'">View Text Information</a>  </div>';

          echo '<div id="showtext'.$ii.'" style="display:none; text-align:left;">';
          foreach ($texts as $row2) {
            ...
            ?>
            <div style="font-size: <?php echo $row2['textSize']?>px;color: <?php echo $row2['color']?>;font-family: <?php echo $row2['font']?>;">
              <?php echo $row2['text']?></div>
              <hr>
              <?php
            }
            echo '</div>';
          ...
          if($custom_uplaod_img->is_custom_uploaded){ 
            ...
            if(!empty($custom_uplaod_img->front_upload_image)){
              echo '<div><a class="btn btn-info" style="color:#FFF;" href="'.base_url('superadmin/download_custom_image/'.$custom_uplaod_img->front_upload_image).'" title="">Download Custom <br>Design</a></div>';
            }
            echo '<br>';

            if(!empty($custom_uplaod_img->back_upload_image)){
              echo '<div class="row text-left"><a class="btn btn-info" style="color:#FFF;" href="'.base_url('superadmin/download_custom_image/'.$custom_uplaod_img->back_upload_image).'" title="">Download Custom <br>Design</a></div>';
            }
      ...
      <td> <?php if($get_design_id=get_design_id($row->order_id,$row->product_id))
        {
          ...
          foreach ($get_design_id as $key ) {
            ...
            <a href="<?php echo base_url().'superadmin/download_design/'.$key->design_id; ?>" class="btn" title="download design" style="color:#fff"><i class="icon-download"></i></a>
        ...
        <?php $ii++; }  ?> 
        <tr>
          <td colspan="8" style="text-align:right !important">Gross Total </td>
          <td> <?php money_symbol(); ?><?php echo $order_user_info->gross_amount ?></td>
        </tr>
        <?php if ($order_user_info->discount != 0) { ?>
        <tr>
          <td colspan="8" style="text-align:right !important">Discount</td>
          <td> - <?php money_symbol(); ?><?php echo $order_user_info->discount ?></td>
        </tr>
        <?php } ?>
        <tr>
          <td colspan="8" style="text-align:right !important">Tax </td>
          <td><?php money_symbol(); ?><?php echo $order_user_info->tax_amount ?></td>
        </tr>                
        <tr>
          <td colspan="8" style="text-align:right !important">Total Amount </td>
          <td><?php money_symbol(); ?><?php echo $order_user_info->total_amount ?></td>
        </tr>
        <?php echo form_open(current_url()) ?>   
        <tr>
          <td colspan="8" style="text-align:right !important">Change Status </td>
          <td style="width:0">
          ...
          <tr>
            <td colspan="8" style="text-align:right !important"></td>
