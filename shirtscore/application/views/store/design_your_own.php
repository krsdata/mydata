...
.fpd-container > .fpd-product-selection > div {
    background-color: #fafafa;
    border: 1px solid #c3c3c3;
    border-radius: 5px;
    box-shadow: 0 1px 0 0 #ffffff, 0 2px 3px 0 rgba(0, 0, 0, 0.1) inset;
    height: 570px;
    outline: medium none;
...
/*.ui-state-default .ui-icon {
    background-image: url("<?php echo base_url() ?>images/ui-icons_888888_256x240.png") !important;
    height: 10px;
    width:10px;
}*/
</style>
...
<?php } ?>
  ...
  <div class="row-fluid" >
          ...
          <span class="span5" style="color: #51A351; font-size: 24px;font-weight: bold;">DESIGN YOUR OWN SHIRT!</span>
       
           <div class="api-buttons clearfix" style="margin-bottom:-20px">
                       
                <div class="price span2" style="padding:8px; min-width:30px;color: #3b5998; margin-left:50px">PRICE:&nbsp;&nbsp;<span class="shirt-price"><?php echo money_symbol(); ?></span><span id="thsirt-price" class="shirt-price"></span>
                 </div>
...
<div class="row-fluid" >
 ...
 <span><button id="upload-button" class="btn btn-warning"> <i class="icon-upload icon-white"></i> Upload Your Own Design</button></span>
 <span class="pull-right" style="margin-right:15px"><button id="image-button" data-original-title="" class="btn btn-success"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class="icon-shopping-cart icon-white"></i> ADD TO CART&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</button></span><!-- Please select front view of product then proceed. -->
  ...
  jQuery(document).ready(function() {
      ...
      var fpd = jQuery('#fpd').fancyProductDesigner({
        ...
        customTextParamters: {x: (f_left+1), y: (f_top+1), colors: "#000", removable: true, resizable: true, draggable: true, rotatable: false, price: 4, boundingBox:{x: f_left, y: f_top, width: f_width, height: f_height}}
      })
...
jQuery('#fpd').find('.fpd-reset').click(function(event) {
  //jQuery('#fpd').find('.fpd-remove').trigger('tap');
  window.location ='<?php echo base_url("store/design_your_own") ?>';
});;
            ...
            reader.onload = function (e) {
              ...
              image.onload = function() {
                ...
                if(image.width < 1024 && image.height < 1024){
                 alert('Uploaded design file is too small. Uploaded design file size should be atleast 1024px.');
                 return false;
               }
            ...
            $('input[name="b_custom_img"]').val(e.target.result);
              ...
              // var fcustomimg =  e.target.result;
               // alert(fcustomimg);
                ...
                fpd.addElement('image', e.target.result, 'my custom design', {colors:$('#colorizable').is(':checked') ? '#000000' : false, zChangeable: true, removable: true, draggable: true, resizable: true, rotatable: true, x: (f_left + 10), y: (f_top + 10), scale: scaling.toFixed(2), price: 8.99, slug: 0, boundingBox:{x: f_left, y: f_top, width: f_width, height: f_height}});
              };
          };
        }
        else {
           ...
           // alert(data);
            ...
            if(jsondata.STATUS==1){
             ...
             if(changedesign_cat_id>0){
              $('.fpd-design-selection div').find('ul').html('<li style="width:100%;padding: 30px 8px;border-bottom:0px;">No Design Found.</li>');
              }
            }
...
<script type="text/javascript">
  function get_col_imgs(col, ind) {
          // alert(col);
          // return false;
          jQuery('#color_ajax_loader').show();
          jQuery.ajax({
                type: "POST",
                data: { action: 'get_content',},
                url: "<?php echo base_url() ?>store/ajax_color_set/"+col,
                success: function(res){
                  jQuery('#color_ajax_loader').hide();
                  // console.log(res);
                  var obj = jQuery.parseJSON(res);
                  if(obj.status === 1){
                     var front = obj.front;
...
<div id="fpd">   
  ...
  <div class="fpd-design">
        ...
        <?php if (!empty($designs1)): ?>
          ...
          <img class="designs" id="design<?php echo $row->id ?>" src="<?php echo base_url() ?>assets/uploads/designs/thumbnail/<?php echo $row->design_image ?>" title="<?php echo $row->design_title; ?>" price="<?php echo $row->price ?>" data-parameters='{"x": 215, "y": 200,  "removable": true, "draggable": true, "rotatable":false, "resizable": true, "price": <?php echo $row->price ?>,"design_id":<?php echo $row->id ?>, "slug": 1 <?php if (!empty($boundingBox)){ ?>, "boundingBox" : <?php echo $box; } ?>,"scale": 0.30}' />
           <?php endforeach ?>
        <?php //else: ?>
        <?php endif; ?>
      ...
      <div  id="color_boxes">
        ...
        <div class="">
         <div class="span12">
          ...
          <span id="color_boxes_data" class=""><?php echo $col_html; ?></span>

          </div>
        </div>
        <div class="" style="height:10px;"><span><strong>Select Your Product Color</strong></span> <span id="color_ajax_loader" style="display:none;">
          <img src="<?php echo base_url() ?>assets/images/ajax-loader.gif" alt="Loader">
          </span></div>
      </div>
    </div>
